...
function PostFirstEntry()
{
    ...
    if( dxr.flags.settings.enemyrespawn <= 0 && dxr.flags.moresettings.reanimation <= 0 ) return;
...
{
    local DXREnemyRespawn er;

    er = DXREnemyRespawn(Find());
    if(er == None) return carc.Level.TimeSeconds + 20;
    return er._GetZombieTime(carc);
}
...
function _SetZombieTime(#var(DeusExPrefix)Carcass carc, float time)
{
    ...
    if(!dxr.flags.IsHalloweenMode() && dxr.flags.moresettings.reanimation <= 0) return;
...
static function SetZombieTime(#var(DeusExPrefix)Carcass carc, float time)
{
    ...
    er = DXREnemyRespawn(class'DXRando'.default.dxr.FindModule(class'DXREnemyRespawn'));
    if(er == None) return;
    er._SetZombieTime(carc, time);
}
#endif
