...
*/
	...
	else if ((invItemTo.IsA('DeusExPickup')) && (DeusExPickup(invItemTo).bCanHaveMultipleCopies))
	{
		...
		if (event.transferCount > DeusExPickup(invItemTo).maxCopies) {
			player.ClientMessage("The story wanted to give you "$event.transferCount$" "$DeusExPickup(invItemTo).ItemName$"s but you can only carry "$DeusExPickup(invItemTo).maxCopies$" - Sorry!",, true);
		...
		if (bSpawned)
		{
			if (event.transferCount > 1)
			{
				...
				if ((DeusExPickup(invItemTo).NumCopies + event.transferCount) > DeusExPickup(invItemTo).maxCopies){
					itemsTransferred = 0;
					player.ClientMessage("Not enough space to receive "$event.transferCount$" "$DeusExPickup(invItemTo).ItemName$"s (Maximum: "$DeusExPickup(invItemTo).maxCopies$")",, true);
				} else {
