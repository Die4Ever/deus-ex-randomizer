...
function bool DXReduceDamage(int Damage, name damageType, vector hitLocation, out int adjustedDamage, bool bCheckOnly)
{
    ...
    newDamage = AdjustCritSpots(newDamage, damageType, hitLocation); //Maybe not strictly necessary?
    oldDamage = newDamage;

    newDamage = ReduceEnviroDamage(newDamage, damageType);

    if ((damageType == 'Shot') || (damageType == 'Sabot') /*|| (damageType == 'Exploded')*/ || (damageType == 'AutoShot')) //GMDX doesn't make armour work on explosions
    {
...
/*
    //GMDX just merges AugEMP right into AugShield
    if (damageType == 'EMP')
    {
        if (AugmentationSystem != None)
...
simulated function DrugEffects(float deltaTime)
{
    ...
    drugEffectTimer = FMax(drugEffectTimer,0.0);
}
...
function int _HealPart(out int points, out int amt, int max, optional int partMax)
{
...
exec function ToggleScope()
{
    ...
    if (camera==None || camera.GetExpectedCameraMode() == camera.CM_FirstPerson){
        //GMDX scope animation logic in first person
        Super.ToggleScope();
    } else {
        //Third person or Fixed Cameras
        //Vanilla scope logic (Just toggle it...)
...
function bool HighlightCenterObjectMultitoolPerk()
{
    ...
    oldFrobTime = FrobTime;

    MaxFrobDistance = 768.0; //Multitool Wireless Strength range
    HighlightCenterObjectMain(); //Do the regular highlight logic, with extended range
    MaxFrobDistance = Default.MaxFrobDistance; //Back to default

    if (HackableDevices(FrobTarget)==None){
        FrobTarget = oldFrobTarget;
        FrobTime = oldFrobTime;
        return False;
    }
...
//Yeah, let's stick some logic related to healing while crouching into here, why not
function HighlightCenterObjectCrouchLogic()
{
    if (bIsCrouching)
    {
...
//GMDX stuff that happens in HighlightCenterObject (Obviously related to highlighting the thing you're aiming at)
function HighlightCenterObjectLadderLogic()
{
    local float rnd;
...
event WalkTexture( Texture Texture, vector StepLocation, vector StepNormal )
{
    ...
    Super.WalkTexture(Texture,StepLocation,StepNormal);
}
...
exec function ParseLeftClick()
{
    ...
    if(!bSpyDroneActive &&
       !bGEPProjectileInFlight &&
       InHand==None &&
       FrobTarget!=None &&
       DeusExPickup(FrobTarget)!=None){

        //Handling this ourselves will allow these items to go through
        //the regular Activate codepath, so bingo will work
        if (CanInstantLeftClick(DeusExPickup(FrobTarget))){
            InstantlyUseItem(DeusExPickup(FrobTarget));
            FrobTarget=None;
        }
    }
...
function SetUpGMDXHardcoreStuff(int diff_val, int stamina_val)
{
...
function setupDifficultyMod()
{
    ...
    if (flagbase!=None){
        ...
        SetUpGMDXHardcoreStuff(diff,stam);
    }
...
state PlayerWalking
{
    ...
    // crouching makes you two feet tall
        ...
        // RANDO: Move speed reduction is reduced based on default health values
