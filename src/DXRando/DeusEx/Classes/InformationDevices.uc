...
function AddToVault()
{
    ...
    if( plaintext != "" ) {
        ...
        plainTag = StringToName(GetTextTag(self));
        log(Self$": plainTag: "$plainTag);
        MarkTextRead(plainTag);
        HXGameInfo(Level.Game).AddNote(vaultString, , True, plainTag);
        return;
    }
...
function CreateInfoWindow()
{
    ...
    if( plaintext != "" ) {
        ...
        if (bAddToVault)
        {
            ...
            log(Self$": plainTag: "$plainTag);
            MarkTextRead(plainTag);
            note = GetNote(plainTag);
            if (note == None)
            {
                note = aReader.AddNote(vaultString,, True);
                SetTextTag(note, plainTag);
                WritePasswordsToNote(note);
            }
        }
        vaultString = "";
...
static function string GetTextTag(#var(prefix)InformationDevices id)
{
    ...
    //TextTag name priority:
    // 1) Actual textTag
    // 2) imageClass
    // 3) plaintextTag (definable when spawning datacubes)
    // 4) Autogenerated MapName_CRC id
    if (id.textTag!=''){
        TextTag = string(id.textTag);
    } else if (id.imageClass!=None){
        TextTag = id.imageClass.default.imageDescription;
        TextTag = class'DXRInfo'.static.ReplaceText(TextTag," ","");
        TextTag = class'DXRInfo'.static.ReplaceText(TextTag,"-","");
        TextTag = class'DXRInfo'.static.ReplaceText(TextTag,":","");
        TextTag = class'DXRInfo'.static.ReplaceText(TextTag,",","");
    } else {
        ...
        //Datacubes using plaintext can define a plaintextTag to simplify identification.
